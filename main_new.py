...
class Lexer:
    ...
    def __init__(self, source: str):
        # Store original source for line mapping
        self.original_source = source
        # Calculate line mapping before preprocessing
        self.line_mapping = {}
        self._create_line_mapping(source)
        # Preprocess source
    ...
    def _create_line_mapping(self, source: str) -> None:
        """Create a mapping from preprocessed line numbers to original line numbers."""
        original_lines = source.splitlines()
        processed_line_num = 1
        
        for i, line in enumerate(original_lines, 1):
            # Store mapping from processed line to original line
            self.line_mapping[processed_line_num] = i
            
            # Only increment processed line if this line would be kept
            if line.strip() and '//' not in line:
                processed_line_num += 1
            elif '//' in line and line.split('//', 1)[0].strip():
                # Line with comment but still has content
                processed_line_num += 1

    def _get_original_line(self, processed_line: int) -> int:
        """Map a preprocessed line number to the original source line number."""
        if processed_line in self.line_mapping:
            return self.line_mapping[processed_line]
        # Fallback: return the processed line if no mapping exists
    ...
    def _add_token(self, token_type: TokenType, value: str = None) -> None:
        ...
        # Map the processed line to the original source line
        orig_line = self._get_original_line(self.line)
        self.tokens.append(Token(token_type, value, orig_line)) 
...
class Interpreter:
    ...
    def evaluate_expression(self, expr: Expression, env: Environment) -> Any:
        ...
        if isinstance(expr, LiteralExpr):
            ...
            right = self.evaluate_expression(expr.right, env)            # Handle string values
